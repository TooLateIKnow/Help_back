...
public class AfhinfoDao extends BaseDao {
    ...
    public void addAfhinfo(Afhinfo afhinfo) throws SQLException {
        ...
        String sql = "INSERT INTO `help`.`afhinfo` (  `userId`, `reqInfo`, `location`, `time`, `username`, `picnum`)  VALUES  (   ?  ,   ?   ,   ?   ,   ?   ,   ?   ,   ?   ) ";
        try {
            ...
            statement.setString(6, afhinfo.getPicnum());
    ...
    public List<Afhinfo> Search( ) throws SQLException {
        List<Afhinfo> afhinfos = null;
        afhinfos = new ArrayList<Afhinfo>();//不可少
        ResultSet resultSet = null;
        String sql = "SELECT   * FROM `help`.`afhinfo` ";
        try {
            connection = getConnection();
            //建立传输SQL语句的statement
            statement = connection.prepareStatement(sql);
            ...
            while (resultSet.next()){
                ...
                info.setPicnum(resultSet.getObject("picnum")==null? "" : resultSet.getString("picnum"));//要判空！！！
                afhinfos.add(info);
            }
        } catch (SQLException e) {
            System.out.println("err="+e);
